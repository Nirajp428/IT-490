...
switch ($type) {
	case "error":
		...
		$client = new rabbitMQClient("RabbitMQDev.ini","logExchangeServer");
		break;
	case "login":
		$request['type'] = $_SESSION['type'];
		$request['email'] = $_SESSION['email'];
		$request['password'] = $_SESSION['password'];
		$client = new rabbitMQClient("RabbitMQDev.ini","backEndServer");
		break;
	case "register":
		$request['type'] = $_SESSION['type'];
	...
	case "APIrequest":
		$request['type'] = $_SESSION['type'];
                $request['movie'] = $_SESSION['movie'];
                $client = new rabbitMQClient("RabbitMQDev.ini","dmzServer");
                break;
